...
async function renderMemberDetail(admin, memberId) {
  ...
  const adjustForm = canEditMember
    ...
    <div class="section">
      ...
      <form method="POST" action="/api/admin/member-action" class="stack-form">
  ...
  const deleteForm = canDeleteMember
    ...
    <div class="section">
      ...
      <form method="POST" action="/api/admin/member-action" onsubmit="return confirm('ยืนยันลบสมาชิกนี้ถาวร? ข้อมูลทั้งหมดจะกู้คืนไม่ได้')">
  ...
  const rows = (rewards || [])
    .map((r) => {
      ...
      const deleteCell = canDelete
        ? `<td style="text-align:center;">
            <form method="POST" action="/api/admin/reward-action" onsubmit="return confirm('ลบของรางวัลนี้?')" style="display:inline;">
              <input type="hidden" name="action" value="delete" />
              <input type="hidden" name="id" value="${r.id}" />
...
async function renderCampaignsTab(admin) {
  ...
  const rows = (creatives || [])
    .map((c) => {
      ...
      const deleteCell = canDelete
        ? `<td style="text-align:center;">
            <form method="POST" action="/api/admin/campaign-action" onsubmit="return confirm('ลบ Campaign นี้?')" style="display:inline;">
              <input type="hidden" name="action" value="delete" />
              <input type="hidden" name="creative_id" value="${c.creative_id}" />
              <button class="btn-small btn-danger">ลบ</button>
            </form>
...
async function renderAdminsTab(admin) {
        ...
        <tr>
          ...
          <td>
            ${
              isSelf
                ? a.role
                : `<form method="POST" action="/api/admin/admin-user-action" class="inline-form">
          ...
          <td style="text-align:center;">
            ${
              isSelf
                ? ''
                : `<form method="POST" action="/api/admin/admin-user-action" onsubmit="return confirm('ลบบัญชี ${a.username}?')" style="display:inline;">
                     <input type="hidden" name="action" value="delete" />
                     <input type="hidden" name="username" value="${a.username}" />
                     <button class="btn-small btn-danger">ลบ</button>
                   </form>`
  ...
  return `
    <div class="section">
      ...
      <form method="POST" action="/api/admin/admin-user-action" class="stack-form">
        <input type="hidden" name="action" value="create" />
