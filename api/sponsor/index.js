...
async function renderBookTab(sponsor, query) {
  ...
  const slotCheckboxes = slotNumbers
    .map((slotNum) => {
      ...
      if (booking) {
        return `<label class="slot-box slot-full">Slot ${slotNum}<br/><span class="hint">ไม่ว่าง</span></label>`;
      }
      availableCount++;
      return `<label class="slot-box slot-available">
        <input type="checkbox" name="slot_numbers" value="${slotNum}" />
        Slot ${slotNum}<br/><span class="hint">ว่าง</span>
      </label>`;
    })
    .join('');

  const contentOptions = approvedContent.map((c) => `<option value="${c.id}">${c.file_name}</option>`).join('');

  const bookingForm = approvedContent.length
    ? `
    <div class="section">
      <h2>3. เลือก Slot ที่ต้องการ (เลือกได้หลายอัน)</h2>
      ...
      <form method="POST" action="/api/sponsor/action?action=create_bookings" id="bookForm">
        ...
        <div style="margin-top:16px; max-width:400px;">
          <label class="hint" style="display:block; margin-bottom:4px;">ไฟล์ที่จะแสดงในทุก Slot ที่เลือก</label>
          <select name="sponsor_content_id" required>${contentOptions}</select>
        </div>
    ...
    <script>
      ...
      const checkboxes = document.querySelectorAll('input[name="slot_numbers"]');
      const totalEl = document.getElementById('totalPrice');
      function updateTotal() {
        const count = document.querySelectorAll('input[name="slot_numbers"]:checked').length;
        totalEl.textContent = 'เลือก ' + count + ' slot — ยอดรวม: ' + (count * pricePerSlot).toLocaleString() + ' บาท';
      }
      checkboxes.forEach((cb) => cb.addEventListener('change', updateTotal));
      document.getElementById('bookForm').addEventListener('submit', (e) => {
        if (document.querySelectorAll('input[name="slot_numbers"]:checked').length === 0) {
          e.preventDefault();
    ...
    <style>
      ...
      .slot-box { border: 1px solid #e5e7eb; border-radius: 8px; padding: 10px; text-align: center; font-size: 13px; cursor: pointer; }
      .slot-full { background: #f0f0f0; color: #9ca3af; cursor: not-allowed; }
...
async function renderBookingsTab(sponsor, query) {
  ...
  return `
    <div class="section">
      ...
      <table>
        <tr><th>Office / Slot</th><th>สัปดาห์</th><th>ไฟล์ที่แสดง</th><th></th><th style="text-align:right;">ราคา</th><th style="text-align:center;">สถานะ</th><th></th></tr>
        ${rows || '<tr><td colspan="7" class="muted">ยังไม่มีการจอง</td></tr>'}
      </table>
    </div>`;
}
